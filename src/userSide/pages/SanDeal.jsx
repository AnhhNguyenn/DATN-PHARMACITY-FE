...
const SanDeal = () => {
    ...
    const [timeLeft, setTimeLeft] = useState({ hours: 28, minutes: 54, seconds: 0 });
    const containerRef = useRef(null);

    useEffect(() => {
        // Filter products from different categories
        if (products.length !== 0) {
            const categories = ['cham-soc-suc-khoe', 'duoc-pham', 'thuc-pham-chuc-nang', 'cham-soc-sac-dep'];
            ...
            const productsWithDiscount = selectedProducts.map(product => ({
                ...
                price: product.price * (1 - (Math.floor(Math.random() * 3 + 1) * 10) / 100),
